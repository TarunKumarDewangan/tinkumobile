...
export default function DataBackupModal({ isOpen, onClose, onRefresh, title, endpoint, typeLabel }) {
  ...
  // Warn user if they try to close/refresh while a restore is in progress
  useEffect(() => {
    if (!loading) return;
    const handler = (e) => {
      e.preventDefault();
      e.returnValue = 'A system restore is in progress. Are you sure you want to leave?';
    };
  ...
  const executeRestore = async () => {
    ...
    try {
      ...
      if (serverMsg) {
        toast.error(`Restore failed: ${serverMsg}. If the database was partially cleared, contact support immediately.`);
      } else {
        toast.error('Restore failed — the database may be in an inconsistent state. Do NOT refresh. Contact support.');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Modal show={isOpen} onClose={onClose} title={title} size="md">
      ...
      {confirmRestore && (
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,.5)',zIndex:9999,display:'flex',alignItems:'center',justifyContent:'center'}} onClick={() => setConfirmRestore(false)}>
          <div style={{background:'#fff',borderRadius:16,padding:'28px 32px',maxWidth:440,width:'90%',boxShadow:'0 20px 60px rgba(0,0,0,.3)'}} onClick={e => e.stopPropagation()}>
            <div style={{fontSize:'1.5rem',fontWeight:800,marginBottom:8}}>⚠️ Confirm Restore</div>
            <p style={{fontSize:'.85rem',color:'#475569',lineHeight:1.6,marginBottom:20}}>
              This will <strong>replace all data</strong> in the system with the backup file contents. 
              Existing records will be deleted and re-inserted. This action <strong>cannot be undone</strong>.
            </p>
            <p style={{fontSize:'.82rem',color:'#dc2626',background:'#fef2f2',padding:'10px 14px',borderRadius:8,marginBottom:20,fontWeight:600}}>
              Make sure you have a current backup downloaded before proceeding.
            </p>
            <div style={{display:'flex',gap:10,justifyContent:'flex-end'}}>
              <button onClick={() => setConfirmRestore(false)} style={{background:'#f1f5f9',border:'1.5px solid #e2e8f0',borderRadius:10,padding:'8px 20px',fontSize:'.82rem',fontWeight:600,cursor:'pointer'}}>Cancel</button>
              <button onClick={executeRestore} style={{background:'#ef4444',border:'none',color:'#fff',borderRadius:10,padding:'8px 20px',fontSize:'.82rem',fontWeight:700,cursor:'pointer'}}>Yes, Restore System</button>
            </div>
          </div>
        </div>
      )}
