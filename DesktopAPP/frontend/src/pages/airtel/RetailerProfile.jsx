...
export default function RetailerProfile() {
    ...
    // "Create Main Account" — manual override to add retailer to main accounts system
    const [showMainAccount, setShowMainAccount] = useState(false);
    const [mainAccountForm, setMainAccountForm] = useState({ name: '', phone: '', type: 'RETAILER', opening_balance: 0, balance_type: 'RECEIVABLE' });
    ...
    const handleCreateMainAccount = async (e) => {
        e.preventDefault();
        setCreatingMainAccount(true);
        try {
            await axios.post('/entities', mainAccountForm);
            toast.success(`Main account "${mainAccountForm.name}" created successfully!`);
    ...
    return (
        <div className="container-fluid py-4">
                    ...
                    <div className="d-flex gap-2 justify-content-end mt-2 text-uppercase fw-bold">
                        ...
                        {isOwner() && (
                            <button
                                ...
                                title="Create a separate account for this retailer in the main accounting system"
                                onClick={() => {
                                    setMainAccountForm({
                                        name: retailer.name + ' (MAIN)',
                                        phone: retailer.msisdn,
                                        type: 'RETAILER',
                                        opening_balance: 0,
            ...
            {/* Create Main Account Modal — Owner Only */}
            <Modal show={showMainAccount} onClose={() => setShowMainAccount(false)} title="CREATE MAIN SYSTEM ACCOUNT">
                <div className="alert alert-info border-0 small mb-3">
                    <strong>ℹ️ Airtel is isolated.</strong> This creates a <em>separate, independent</em> account
                    in the main accounting system. It will NOT be auto-linked to Airtel drops or recoveries.
                    Rename it to avoid confusion (e.g. add " (MAIN)").
                </div>
                <form onSubmit={handleCreateMainAccount}>
                    <div className="row g-3">
                        <div className="col-12">
                            <label className="form-label x-small text-uppercase fw-bold">Account Name *</label>
                            <input
                                type="text"
                                className="form-control fw-bold text-uppercase"
                                required
                                value={mainAccountForm.name}
                                onChange={e => setMainAccountForm({...mainAccountForm, name: e.target.value.toUpperCase()})}
                            />
                        ...
                        <div className="col-md-6">
                            ...
                            <select
                                ...
                                value={mainAccountForm.type}
                                onChange={e => setMainAccountForm({...mainAccountForm, type: e.target.value})}
                            >
                        ...
                        <div className="col-md-6">
                            ...
                            <div className="input-group">
                                ...
                                <input
                                    ...
                                    step="0.01"
                                    className="form-control fw-bold"
                                    value={mainAccountForm.opening_balance}
                                    onChange={e => setMainAccountForm({...mainAccountForm, opening_balance: e.target.value})}
                                />
                            </div>
                        </div>
                        <div className="col-md-6">
                            ...
                            <select
                                ...
                                value={mainAccountForm.balance_type}
                                onChange={e => setMainAccountForm({...mainAccountForm, balance_type: e.target.value})}
                            >
                    ...
                    <div className="d-flex gap-2 mt-4">
                        <button type="button" className="btn btn-light flex-grow-1" onClick={() => setShowMainAccount(false)}>Cancel</button>
                        <button type="submit" className="btn btn-primary flex-grow-1 fw-bold text-uppercase" disabled={creatingMainAccount}>
                            {creatingMainAccount ? 'Creating...' : '✓ Create Account'}
                        </button>
                    </div>
                </form>
            </Modal>
