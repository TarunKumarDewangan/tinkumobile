...
export default function GroupSummary() {
    ...
    const categoryGroups = filteredEntities.reduce((acc, ent) => {
        ...
        if (type === 'CUSTOMER' || type === 'SHOP_CUSTOMER') {
            adjustedBalance = (parseFloat(ent.sales_balance || 0) + parseFloat(ent.works_balance || 0));
        }
    ...
    const totalDebit = showCategoryTotals
        ...
        : filteredEntities.reduce((s, e) => {
            const isCustomer = e.type === 'CUSTOMER' || e.type === 'SHOP_CUSTOMER';
            const bal = isCustomer ? (parseFloat(e.sales_balance || 0) + parseFloat(e.works_balance || 0)) : e.net_balance;
    ...
    const renderCategoryTradeTypeBadges = (catName) => {
        ...
        if (catName === 'CUSTOMER' || catName === 'SHOP_CUSTOMER') {
            badges.push({ label: 'SALES', bg: 'success' });
        } else if (catName === 'REPAIR') {
            badges.push({ label: 'REPAIR', bg: 'warning text-dark' });
        } else if (catName === 'SUPPLIER' || catName === 'DISTRIBUTOR') {
            badges.push({ label: 'PURCHASE', bg: 'primary' });
        } else {
            badges.push({ label: 'WORKS', bg: 'secondary' });
        }
                                        ...
                                        <tr key={ent.id} className="tally-row">
                                            ...
                                            <td className="text-end fw-bold py-2 pe-3">
                                                {(() => {
                                                    ...
                                                    const bal = isCustomer ? (parseFloat(ent.sales_balance || 0) + parseFloat(ent.works_balance || 0)) : ent.net_balance;
