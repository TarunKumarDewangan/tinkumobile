...
class UserController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        // Audit log
        ActivityLog::log('USER_CREATED', $authUser, "User {$user->name} ({$user->email}) created with role {$data['role']}");
    ...
    public function update(Request $request, User $user)
    {
        ...
        // Audit log — log role change if role was updated
        if (isset($data['role'])) {
            ActivityLog::log('USER_ROLE_CHANGED', $authUser, "User {$user->name} ({$user->email}) role updated to {$data['role']}");
        } else {
            ActivityLog::log('USER_UPDATED', $authUser, "User {$user->name} ({$user->email}) profile updated");
        }
    ...
    public function destroy(Request $request, User $user)
    {
        ...
        // Capture user info before delete
        $userName = $user->name;
        $userEmail = $user->email;
        $user->delete();
        // Audit log
        ActivityLog::log('USER_DELETED', $authUser, "User {$userName} ({$userEmail}) deleted");
