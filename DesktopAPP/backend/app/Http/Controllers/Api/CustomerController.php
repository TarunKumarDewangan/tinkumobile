...
class CustomerController extends Controller
{
    ...
    public function store(Request $request) {
        ...
        // Application-layer uniqueness check (respects soft deletes)
        $existing = Customer::whereNull('deleted_at')->where('phone', $data['phone'])->first();
        if ($existing) {
            return response()->json(['message' => 'Phone number already in use by another customer.'], 422);
        }
    ...
    public function update(Request $request, Customer $customer) {
        ...
        // Check phone uniqueness (excluding soft-deleted records and current customer)
        if (isset($data['phone'])) {
    ...
    public function getHistory(Request $request, Customer $customer) {
        // Shop scope check — prevent IDOR across shops
        $user = $request->user();
        if (!$user->isOwner() && !$user->isAdmin() && $customer->shop_id !== $user->shop_id) {
