        ...
        if (!$entityId && $this->entity_type && $this->entity_id) {
            ...
            if ($related) {
                if ($this->entity_type === \App\Models\RepairRequest::class) {
                    ...
                    $entity = \App\Models\Entity::where('relation_type', \App\Models\Customer::class)
                        ->where('relation_id', $related->customer->id)
                        ->first();
                    if (!$entity) {
                        $entity = \App\Models\Entity::where('name', $related->customer->name)->first();
                    }
                    if ($entity) $entityId = $entity->id;
                } elseif (method_exists($related, 'supplier') && $related->supplier) {
                    $entity = \App\Models\Entity::where('relation_type', \App\Models\Supplier::class)
                        ->where('relation_id', $related->supplier->id)
                        ->first();
                    if (!$entity) {
                        $entity = \App\Models\Entity::where('name', $related->supplier->name)->first();
                    }
                    if ($entity) $entityId = $entity->id;
