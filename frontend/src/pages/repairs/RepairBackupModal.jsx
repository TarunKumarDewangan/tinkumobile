...
export default function RepairBackupModal({ isOpen, onClose, onRefresh }) {
  const [tab, setTab] = useState('export'); // 'export' or 'import'
  const [backupType, setBackupType] = useState('full'); // 'full' or 'duration'
  ...
  const handleExport = async () => {
    if (backupType === 'duration' && (!dates.start_date || !dates.end_date)) {
      return toast.warning('Please select both start and end dates for duration backup.');
    }
    ...
    try {
      ...
      const response = await api.get('/repairs/backup', {
        params,
      ...
      let filename = 'repair_backup.json';
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const res = await api.post('/repairs/restore-backup', formData, {
  ...
  return (
    <Modal show={isOpen} onClose={onClose} title="Repair Backup & Restore" size="md">
      <div className="mb-4">
      ...
      {tab === 'export' ? (
        <div className="px-2">
          <div className="mb-4">
            <div className="form-check mb-2">
              ...
              <div className="text-muted small">Download all repairs currently in the system.</div>
            </div>
            ...
            <div className="form-check">
              ...
              <div className="text-muted small">Download repairs submitted within a specific date range.</div>
            </div>
          </div>
        ...
        <div className="px-2">
          <div className="alert alert-info py-2 small">
            <strong>Note:</strong> Uploading a backup will update existing records that match by ID and insert any new records found in the file. No existing records will be deleted.
          </div>
