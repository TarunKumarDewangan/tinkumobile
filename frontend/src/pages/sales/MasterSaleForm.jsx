...
export default function MasterSaleForm() {
  const { id } = useParams();
  ...
  const isImeiRow = (item) => {
    if (!item) return false;
    const prodId = item.product_id || item.selection_id;
    if (!prodId) return false;
    const p = products.find(prod => prod.product_id === prodId || prod.id === prodId);
    if (!p) return false;
  ...
  const loadProducts = async (shopId) => {
    if (!shopId) return;
    try {
      const params = { shop_id: shopId, category_group: 'master', group_by_config: 'false' };
  ...
  const addItem = () => setItems([...items, { selection_id: '', product_id: '', product_name: '', imei: '', imeis: [''], ram: '', storage: '', color: '', description: '', quantity: 1, unit_price: '', base_price: 0, min_selling_price: 0, max_selling_price: 0, apply_gst: false }]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const finalItems = [];
      items.forEach(item => {
        if (isImeiRow(item) && item.quantity > 1) {
      ...
      <div className="page-header mb-3 d-flex justify-content-between align-items-center">
        <div className="text-uppercase">
           <h2 className="mb-0 fw-bold">
              {id ? '✍️ EDIT MASTER SALE' : '➕ MASTER SALE ENTRY'}
           </h2>
           <p className="text-muted small mb-0">RECORD PRODUCT SALES ACROSS ALL PRODUCT CATEGORIES AND GST</p>
        </div>
        <button onClick={() => navigate('/sales?category_group=master')} className="btn btn-outline-secondary btn-sm text-uppercase fw-bold">← Back to List</button>
      </div>
                            ...
                            {items.map((item, i) => {
                                ...
                                return (
                                    <tr key={i}>
                                            ...
                                            <div className="row g-1 text-uppercase">
                                                <div className="col-12 mb-1">
                                                    <label className="x-small fw-bold text-primary mb-0">
                                                        {isImeiRow(item) ? 'IMEI/SERIAL NUMBER' : 'SERIAL / BATCH (OPTIONAL)'}
                                                    </label>
                                                    <input 
                                                        type="text" 
                                                        list={`stockImeis-${i}-0`}
                                                        className="form-control form-control-sm fw-bold border-primary shadow-sm" 
                                                        placeholder={isImeiRow(item) ? 'ENTER 15-DIGIT IMEI...' : 'ENTER SERIAL OR BATCH...'} 
                                                        value={item.imei || ''} 
                                                ...
                                                {isImeiRow(item) && item.quantity > 1 && Array.from({ length: item.quantity - 1 }).map((_, imeiIdxPlusOne) => {
                                        ...
                                        <td className="pe-3 text-center" style={{ width: '40px' }}>
